...
let textOverlay3; // intersection point 정보 표시
...
function intersectCirclewithLineSegment(circle, line) {
    const [cx, cy, r] = circle; // circle: [centerX, centerY, radius]
    const [x1, y1, x2, y2] = line; // line: [x1, y1, x2, y2]
    const dx = x2 - x1;
    const dy = y2 - y1;
    const a = dx * dx + dy * dy;
    ...
    if (discriminant < 0) {
        updateText(textOverlay3, "No intersection"); // 교차점이 없음
        return []; 
        ...
        if (points.length == 1) {
            updateText(textOverlay3, "Intersection Points: " + "1 " + 
                "Point 1: ("+ points[0][0].toFixed(2) + ", " + points[0][1].toFixed(2) + ")");
        }
        else { // points.length == 2
            updateText(textOverlay3, "Intersection Points: " + "2 " + 
                "Point 1: ("+ points[0][0].toFixed(2) + ", " + points[0][1].toFixed(2) + "), " +
                "Point 2: ("+ points[1][0].toFixed(2) + ", " + points[1][1].toFixed(2) + ")");
        }
...
function render() {
    ...
    for (let line of lines) {
        if (num == 0) { 
            ...
            // circle을 그리기 위해 중심과 반지름 계산
            let radius = calculateRadius([line[0], line[1]], [line[2], line[3]]);
            let vertices = [];

            // circle의 각 점 좌표 계산
            for (let i = 0; i <= numSegments; i++) {
                let angle = (i / numSegments) * 2 * Math.PI;
                let x = line[0] + radius * Math.cos(angle);
                let y = line[1] + radius * Math.sin(angle);
                vertices.push(x, y);
            }
    ...
    if (isDrawing && startPoint && tempEndPoint && lines.length == 0) {
        ...
        let radius = calculateRadius(startPoint, tempEndPoint);
        let vertices = [];

        for (let i = 0; i <= numSegments; i++) {
            let angle = (i / numSegments) * 2 * Math.PI;
            let x = startPoint[0] + radius * Math.cos(angle);
            let y = startPoint[1] + radius * Math.sin(angle);
            vertices.push(x, y);
        }
    ...
    // Intersection point 계산 및 표시
    if (lines.length == 2) {
        let circle = [lines[0][0], lines[0][1], calculateRadius([lines[0][0], lines[0][1]], [lines[0][2], lines[0][3]])];
        let line = lines[1];
